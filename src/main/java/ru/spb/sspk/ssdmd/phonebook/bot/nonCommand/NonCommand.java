package ru.spb.sspk.ssdmd.phonebook.bot.nonCommand;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import ru.spb.sspk.ssdmd.phonebook.bot.TelegramBot;
import ru.spb.sspk.ssdmd.phonebook.exception.IllegalSettingsException;
...
public class NonCommand {
    ...
    private void saveUserSettings(Long chatId, Settings settings) {
        if (!settings.equals(TelegramBot.getDefaultSettings())) {
            TelegramBot.getUserSettings().put(chatId, settings);
        } else {
            TelegramBot.getUserSettings().remove(chatId);
        }
    }
